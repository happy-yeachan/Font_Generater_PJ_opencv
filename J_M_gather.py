...
def gather(filename, tmp_list):
    src = cv2.imread('./test2/' + filename + '.jpg')
    ...
    try:
        for i in range(1, len(res)):
            ...
            roi = img270 = cv2.rotate(roi, cv2.ROTATE_90_COUNTERCLOCKWISE)
            cv2.imwrite('./test2/' + tmp_list[index] + '.jpg', roi)
            # cx, cy = centroids[i]
            index += 1      
    except:
